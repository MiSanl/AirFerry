...
```
          ...
          └────┬───────────────────┘
               ▼
        ┌──────────────────────────────┐
        │ 2–6 解码 worker（并行）        │  共享 ZXing-C++；全帧冷启动/多 bbox ROI 热路径
        └────┬─────────────────────────┘
             ▼
        ┌──────────────────────────┐
...
> **解码/摄入分离**：完成时先停止摄入再 `assemble()`。会话切换和进度快照都在 ingest 锁内完成。bbox 锁定后优先 ROI；连续 3 次整组 ROI miss 才回退全帧重锁。首次只锁到部分 tile 时，每 30 次 ROI 成功安排下一帧低频补发现（不在同一帧重复做 ROI + 全帧）；连续 60 次未命中的槽位退休，重现后再发现。Android/Windows 策略一致。
