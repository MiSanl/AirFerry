...
| **安全闸门** OTI 校验 | `core/raptorq-core/src/meta.rs:107` | `ObjectMeta::validate`——接收前必跑，防 panic-on-abort |
...
| 压缩分发 + 解压炸弹防护 | `core/qr-protocol/src/compress.rs:76,107` | `compress_with` / `decompress_with_limit` |
...
| 断点状态序列化 | `core/transfer-engine/src/resume.rs` + `receiver.rs` | `ResumeState`（serde-gated JSON）；JSON 输入/输出封顶 128 MiB，恢复前校验 OTI、坐标、符号数/字节预算；`ReceiverSession::save_state` / `restore` |
| **JNI 绑定（Android）** | `core/transfer-engine/src/jni.rs:67` | `receiverIngest` 返回**packed jlong**（非 JSON，见 SPEC §7）；打包逻辑三端共享 `ingest_status.rs`；`receiverLastAssembleError` 暴露组装/解压失败原因 |
...
| 压缩 worker（离主线程） | `apps/sender/src/workers/compress.worker.ts` | 读→压缩→CRC32→会话 ID。`wasm-init`（可无 zstd）始终解锁队列；发送带 `jobId`/epoch，过期任务不 post。`1×text` → `processText({ jobId, text, name })`（name 规范化进描述符）；否则 `processFiles` |
...
| **★网页接收端（v1.1.6）** | `apps/sender/src/pages/ReceivePage.tsx` | 接收端主页面：相机 `getUserMedia`→`requestVideoFrameCallback` 取帧（**1080 全分辨率，绝不 downscale**——降分辨率会缩小 QR 格导致 zxing 更难解）→QR worker→receive worker→结果分流（文字/包/单文件）。**进度面板对齐 Android `ScanActivity`**：顶部**横向长条形进度条** + 大号百分比 + 始终可见的信息卡（文件名/大小「原~压缩后」/已识别符号 `receivedSymbols/totalSymbols`/解码速率/用时@线上吞吐）。进度基于 receivedSymbols（fountain 可超 K，clamp 100），状态文本复用 Android `computeStatusText` 语义（等待/同步/接收/恢复/完成），滑动窗口速率复用 Android `RATE_WINDOW_MS=3000`/`RATE_MIN_DT_MS=300`。**⚠️ 三个速率指标都是 3 秒滑动窗口平均**：`decodePerSec`（QR 码/秒，用前端 `decodedCodesRef` 每 payload+1，非 RaptorQ `decoded_symbols`——后者整块跳变会长时间为 0）、`recentWireBps`（字节/秒）、`avgCodesPerFrame`（每帧码数 = 窗口内 Δ码数/Δ帧数，非整会话累计）。**⚠️ 线上吞吐需 `progress_json` 的 `symbol_size`**（v1.1.6 新增，`Progress` 加 `symbol_size` 字段，三端 jni/wasm/cffi 同步输出），否则按 1 字节/符号算错。**已移除调试浮层**（`dbg()` 仅 console.log）。`initWorkers` 每次重建会 **terminate 旧 worker 对**（否则反复「再接收一次」泄漏 qr+receive WASM worker）。**⚠️ `applyStatus` 必须声明在 `initWorkers` 之前**（useCallback deps 引用 → `const` TDZ）。web 入口 `apps/web/src/receiver.tsx` + `apps/web/receiver.html`。详见 §2.5「网页接收端」 |
...
| **receive worker** | `apps/sender/src/workers/receive.worker.ts` | 持 `ReceiverSessionWasm` 单例，**串行** ingest（receiver 非线程安全）；jobId/epoch 防过期；**双重引导**：batch 含 descriptor 走 `from_descriptor`（完整验证 OTI）；batch 无 descriptor 时走**缓存引导**（`new(sid_lo,sid_hi)` = `new_pending`，从任意数据帧头解析 session_id）——Rust 缓存 descriptor 前数据帧，descriptor 到达即确认 OTI 并重放，**任意码（非必须左上角 descriptor）都能开始建立传输**。**⚠️ 帧头 session_id 是 16B 大端 `[4..20]`：`[4..12]`=高 64 位、`[12..20]`=低 64 位；`new(lo,hi)` 必须传 `new(低64, 高64)`**——曾把高低位反了，导致缓存引导 session 的 sid 与 descriptor 不同、descriptor 被拒，表现为「看到解码了但建立不了文件流」。**首码进度预估（参考 Android `estimatedTotalSymbols`）**：meta 确认前 `progress_json` 的 `total_symbols`=0，UI 会卡在 0%；receive worker 从帧头 `[32..36]` 读 total_symbols 预估填入 snapshot，`computePct` 在 `!metaConfirmed` 时 cap 到 15%（不过度承诺），首码即显示「正在同步…已缓存 N ~x%」。→批量 `ingest`→`assemble_raw`→解压→解析。消息：`init`/`frames`/`assemble`→`status`/`meta`/`result`/`error`。`status` 携带 `snapshot`（`progress_json` 的 `ProgressSnapshot`）+ `nowMs` 供 UI 算速率/进度 |
...
| **内容库（映射去重）** | `app/.../scan/ContentStore.kt` | `files/store/blobs/<hh>/<sha256>` + `index.json`；同内容一份；详情/分享/列表共用 blob。索引损坏会备份为 `index.corrupt.*.json` 并停止写入，禁止静默清空覆盖。启动 `migrateLegacyReceivedIfNeeded`。Windows：`Bundle/ContentStore.cs` |
...
| 解压 OOM / 崩溃 | `compress.rs:107 decompress_with_limit` | 炸弹上限未封顶到 `original_size` |
...
3. **版本号/Release 混用（历史教训）**：README/dist/workflow 曾出现 v1.0.0/v1.0.1/v1.1.0 漂移（含 `windows.yml` 的 `VER` 滞后）。**当前权威版本 `1.1.4`**（versionCode=9）。改版本时按 §2.8 第 5 条 + §2.9 全表同步，勿只改 `package.json`。
...
8. **Parcel dev-server 安全回移**：Plasmo 0.90.5 依赖 Parcel 2.9.3，直接 override 到 2.16.x 会触发私有 core adapter / BundleGraph 不兼容。`apps/sender/scripts/patch-parcel-dev-server.cjs` 在 postinstall 中把通配 CORS 改为只允许 Chrome/Firefox 扩展 origin，并钉 `@parcel/core=2.9.3` 防 npm 混装两套 core。上游版本升级时补丁会 fail closed，必须重审；不要为追求 `npm audit` 数字而删除回移或强升 Parcel。
