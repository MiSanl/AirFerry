...
> **解码/摄入分离**：完成时先停止摄入再 `assemble()`。会话切换和进度快照都在 ingest 锁内完成。bbox 锁定后优先 ROI；连续 3 次 ROI miss 才回退全帧重锁。首次只锁到部分 tile 时，每 30 次 ROI 成功低频补发现，锁满 4 个后停止；Android/Windows 策略一致。
