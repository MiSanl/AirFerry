...
3. **版本号/Release 混用（历史教训）**：README/dist/workflow 曾出现版本漂移。**当前权威版本 `1.2.11`**（versionCode=25）。v1.2.11 确认 Android 专项验证通过后直接进入全量发布，Pages 未启用时保持手动触发；v1.2.9 新增并修复全量 `release-all` CI，统一构建扩展、网页、Android、Windows 接收端及 Windows 发送端；v1.2.8 移除 Web WASM 的 SIMD 编译依赖，兼容禁用 WebAssembly SIMD 的加固/虚拟化 Chromium；构建目录名 `simd` 为兼容保留。协议仍为 v1.2.0 引入的 descriptor v5（compress-then-segment）。Windows workflow 已移除硬编码 `VER`，只能由现有 `release_tag` 派生并核对 tag commit；改版本时仍须按 §2.8 第 5 条同步代码中的版本源。
