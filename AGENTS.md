...
| ★**截图式源选择器** | `apps/windows/AirFerry.Windows/Views/RegionPicker.cs` + `RegionPickerWindow.xaml(.cs)` | DeviceSelectView「🖥 屏幕捕获」入口 → 每显示器一个全屏半透明覆盖窗（`EnumDisplayMonitors` 物理矩形 + `SetWindowPos` 钉位，各自 `PixelsPerDip` 做 DIP↔物理换算，混合 DPI 多屏各自正确）；拖动=区域（≥32×32），单击=选窗口（<6px 位移算 click），Esc 取消；共享 `RegionPickerSession` + `TaskCompletionSource` 返回 `ScanSource?`。⚠️ **不能用 `WindowFromPoint` 找目标窗口**——覆盖层自身 Topmost 且可命中，永远返回自己；用 `EnumWindows` 按 Z 序扫第一个「非本进程/可见/非工具窗/非 cloaked/≥64px 且含光标」的窗口。`PickAsync` 是**非 async 的 TCS 方法**：覆盖窗创建/`Show()` 中途抛异常时先 `TryComplete(null)` 关闭已显示窗口再**重抛**（同步异常发生在 Task 返回前，调用方 `ScreenCapture_Click` 是 async void——无兜底即崩进程；现整段 try/catch + `UiMessages.ErrorAsync` 兜底） |
