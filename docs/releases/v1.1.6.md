...
| `airferry-receiver-web-v1.1.6.zip` | **网页接收端**静态站点：解压后部署到任意静态托管即可用浏览器摄像头接收（`receiver.html`）。v1.1.6 起**独立打包**，与发送端 web zip 分离，可独立部署。`getUserMedia` 需 HTTPS / localhost |

> **网页接收端**无需安装：打开网页版（`airferry-receiver-web-v1.1.6.zip` 内的 `receiver.html`，或部署后的站点）即可用浏览器摄像头接收。
