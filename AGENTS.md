...
| ★**截图式源选择器** | `apps/windows/AirFerry.Windows/Views/RegionPicker.cs` + `RegionPickerWindow.xaml(.cs)` | DeviceSelectView「🖥 屏幕捕获」入口 → 每显示器一个全屏半透明覆盖窗（`EnumDisplayMonitors` 物理矩形 + `SetWindowPos` 钉位，各自 `PixelsPerDip` 做 DIP↔物理换算，混合 DPI 多屏各自正确）；拖动=区域（≥32×32），单击=选窗口（<6px 位移算 click），**右键=选整个屏幕**（`OnRightUp` 直接以本覆盖窗的显示器矩形完成——全屏应用/游戏首选路径：无边框游戏会因覆盖层抢焦点而最小化、独占全屏窗口既高亮不到也 `PrintWindow` 不到，整屏区域捕获可绕开两者），Esc 取消；覆盖层是**输入屏障**——右/中/X 键与滚轮全部 `e.Handled` 吞掉（`SwallowMouseButton`），只有左键参与选择，防任何输入漏到下层应用；**输入坐标一律 `GetCursorPos` 全局物理坐标**（`CursorPhysical()`，勿改回 `e.GetPosition(this)`+DIP 换算——拖拽经鼠标捕获合法跨屏/跨 DPI，窗口本地 DIP 外推正是跨屏拖框坏掉的根因），渲染锚点为窗口实际矩形回读（`SyncOrigin`，Loaded/OnDpiChanged 重锚）；共享 `RegionPickerSession` + `TaskCompletionSource` 返回 `ScanSource?`，**完成选择后覆盖层延迟 300ms 才关闭**（`TryComplete` 用 DispatcherTimer）——吞掉双击习惯的尾随第二击，否则它会穿透进刚选中的应用（浏览器里表现为「点了文字」）。⚠️ **不能用 `WindowFromPoint` 找目标窗口**——覆盖层自身 Topmost 且可命中，永远返回自己；用 `EnumWindows` 按 Z 序扫第一个「非本进程/可见/非工具窗/非 cloaked/≥64px 且含光标」的窗口。`PickAsync` 是**非 async 的 TCS 方法**：覆盖窗创建/`Show()` 中途抛异常时先 `TryComplete(null)` 关闭已显示窗口再**重抛**（同步异常发生在 Task 返回前，调用方 `ScreenCapture_Click` 是 async void——无兜底即崩进程；现整段 try/catch + `UiMessages.ErrorAsync` 兜底） |
...
3. **版本号/Release 混用（历史教训）**：README/dist/workflow 曾出现版本漂移。**当前权威版本 `1.2.3`**（versionCode=17）。v1.2.3 为全面修复/加固批次：Windows UI 重做（WPF-UI Fluent 主题）+ 屏幕区域/窗口捕获 + 采集卡统一枚举、Android 主线程 IO 全面下沉与销毁后回调崩溃修复、网页接收端 sessionGuard（防环境二维码锁死 + 会话失配重锁）、Rust 核心 zstd 窗口钳制（windowLog≤23）与接收完成后状态增长防护。v1.2.2 为发送端普通/全屏播放增加「停止播放」，停止时先取消渲染循环、再释放 WASM 会话并返回参数页；同时分离 Windows push 质量门与手动 tag 发布的 concurrency group，避免互相取消。协议仍为 v1.2.0 引入的 descriptor v5（compress-then-segment）。descriptor v5 取代撤回的早期 v1.2.0 预发布构建所用 v4（8 MiB 原文段 + 逐段压缩），接收端 fail-closed 拒绝旧 v4——预发布构建与正式版无法互传大文件分段。Windows workflow 已移除硬编码 `VER`，只能由现有 `release_tag` 派生并核对 tag commit；改版本时仍须按 §2.8 第 5 条同步代码中的版本源。
