...
3. **版本号/Release 混用（历史教训）**：README/dist/workflow 曾出现版本漂移。**当前权威版本 `1.2.7`**（versionCode=21）。v1.2.7 新增 Windows 持续接收模式（完成不停扫、直写目标文件夹、SHA-256 复验去重，见 §3.4）与网页接收端屏幕/标签页捕获来源（getDisplayMedia，见 §2.5/§3.2）；v1.2.6 整理 Windows 接收结果页层级；v1.2.5 更新全平台图标并统一 Windows 扫描来源。协议仍为 v1.2.0 引入的 descriptor v5（compress-then-segment）。Windows workflow 已移除硬编码 `VER`，只能由现有 `release_tag` 派生并核对 tag commit；改版本时仍须按 §2.8 第 5 条同步代码中的版本源。
